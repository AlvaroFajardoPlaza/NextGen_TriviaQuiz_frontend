...
export class GenerateTriviaCardComponent {
	...
	registeredUser$: Observable<User> = this._authSvc.User$;
	token$: Observable<any> = this._authSvc.token$;

	categories$: Observable<Array<any>> = this._triviaSvc.getCategories().pipe(
		tap((categories) => {
	...
	cargarDatosCategoriasForm(categories: Array<Array<any>>): void {
		for (const [id, value] of categories) {
			this.categorizedForm.setControl(
				id,
	...
	async onSubmit() {
		console.log(
			'Valores que enviamos en el submit del formulario: ',
		...
		if (!(counter >= 3)) {
			...
			try {
				if (this.categorizedForm.valid) {
					...
					queryParams.categoriesSelected =
						JSON.stringify(paramsAsArray);
	...
	goToRandomTrivia() {
		if (this.token$ == null) {
			...
			return this._router.navigate(['trivia']);
		}
	}
}
